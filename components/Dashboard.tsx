...
import { 
  Car, ParkingSquare, Zap, Wrench, History, Package, BarChart3, ShieldCheck, Lock, MonitorPlay
} from 'lucide-react';
...
const MENU_ITEMS = [
  { id: 'BOX_REPAIR', title: 'Box Repair', sub: '(GWM 盒子维修)', icon: Wrench, perm: 'Box repair', color: 'bg-blue-600' },
  { id: 'PARKING', title: 'Parking', sub: '(GWM 停车处)', icon: ParkingSquare, perm: 'Parking', color: 'bg-indigo-600' },
  ...
  { id: 'USER_MANAGEMENT', title: 'Management', sub: '(权限管理)', icon: ShieldCheck, perm: 'USER PERMISSION MANAGEMENT', color: 'bg-rose-600' },
];
...
export default function Dashboard({ user, onNavigate }: DashboardProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto pt-10">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-white mb-2">Main Menu</h2>
        <p className="text-slate-400">Select a module to proceed</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {MENU_ITEMS.map((item) => {
          const allowed = hasPermission(item.perm);
          const Icon = item.icon;
          if (!allowed && item.id !== 'USER_MANAGEMENT') return null;
          ...
          return (
            <button
              ...
              className={`relative group flex flex-col items-center justify-center p-8 rounded-2xl transition-all duration-300 border border-slate-700/50 shadow-2xl backdrop-blur-sm ${allowed ? 'hover:scale-105 hover:shadow-blue-500/20 cursor-pointer bg-slate-800/80' : 'opacity-50 cursor-not-allowed bg-slate-900'}`}
            >
