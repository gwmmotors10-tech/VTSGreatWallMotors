  ...
  const handleExport = () => {
    ...
    if (activeTab === 'FAST_REPAIR_HISTORY') {
      ...
      dataToExport = filterByDate(vehicles, 'createdAt').map(v => ({
        "VIN number": v.vin,
        "inclusão": formatDateTime(v.createdAt),
        "conclusão (AOFF)": v.finishedAt ? formatDateTime(v.finishedAt) : 'Pending',
        "Lead time (minutos)": calculateLeadTime(v.createdAt, v.finishedAt).replace(' min', ''),
        "Model": v.model,
        "Color": v.color,
        "Origin": v.origin,
        "Destiny": v.destination,
        "Responsible": v.responsible?.join(', ') || 'N/A',
        "constraints": v.missingParts?.join(', ') || 'None'
      }));
